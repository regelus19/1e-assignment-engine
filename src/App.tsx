...
import { AssignmentWarning, CurrentShiftState, FinalizedShiftSnapshot, ForecastEvent, NurseStaff, OnCallProviders, OperationalEvent, PatientRoom, PlanBaseline, PlanningWorkspace } from './types';
...
export const App:React.FC=()=>{
 const[activeTab,setActiveTab]=useState<Tab>('current'),[planningWorkspace,setPlanningWorkspace]=useState<PlanningWorkspace>(()=>StorageService.loadPlanningWorkspace(tomorrowIso(),MRS_CONFIG.target)),[forecastEvents,setForecastEvents]=useState<ForecastEvent[]>(()=>StorageService.loadForecast()),[warnings,setWarnings]=useState<AssignmentWarning[]>([]),[fitScore]=useState(100),[currentShift,setCurrentShift]=useState<CurrentShiftState>(()=>StorageService.loadCurrentShift()),[events,setEvents]=useState<OperationalEvent[]>(()=>StorageService.loadOperationalEvents()),[planBaseline,setPlanBaseline]=useState<PlanBaseline|null>(()=>StorageService.loadPlanBaseline()),[history,setHistory]=useState<FinalizedShiftSnapshot[]>(()=>StorageService.loadHistory());
 const {targetDate:date,shiftType,mtState,pctState,roster,rooms,onCall,pmOnCall,currentMRS,projectedMRS}=planningWorkspace;
 const updatePlanningWorkspace=(patch:Partial<PlanningWorkspace>)=>setPlanningWorkspace(current=>StorageService.savePlanningWorkspace({...current,...patch}));
 ...
 const reset=()=>{StorageService.resetToDefaults();setPlanningWorkspace(StorageService.loadPlanningWorkspace(tomorrowIso(),MRS_CONFIG.target));setForecastEvents(StorageService.loadForecast());setWarnings([]);setCurrentShift(StorageService.loadCurrentShift());setEvents(StorageService.loadOperationalEvents());setPlanBaseline(StorageService.loadPlanBaseline());setHistory(StorageService.loadHistory());setActiveTab('current')};
