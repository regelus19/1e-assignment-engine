...
export interface NurseStaff {
  ...
  actualRecallTime?: string;
  coveringMT?: boolean;
}

export interface OnCallProviders {
  intensivist: string;
  cardiothoracic: string;
  acuteMI: string;
  cardiology: string;
  hospitalist: string;
}
...
export interface AssignmentWarning {
  type: 'CAPABILITY' | 'DIALYSIS_ROOM' | 'WORKLOAD_RATIO' | 'GEOGRAPHY' | 'SAFETY_ROOM';
...
export interface NurseRecommendationDetail {
  nurseId: string;
  nurseName: string;
...
export interface RecommendationResult {
  ...
  nurseDetails: NurseRecommendationDetail[];
  warnings: AssignmentWarning[];
  fitScore: number;
  fitLabel: 'HIGH FIT' | 'MODERATE FIT' | 'REQUIRES REVIEW';
  unassignedRooms: string[];
}

export interface FinalizedShiftSnapshot {
  id: string;
  date: string;
  shiftType: 'Day' | 'Night';
  finalizedAt: string;
...
export interface PlanBaseline extends FinalizedShiftSnapshot {
  savedAt: string;
}

export type OperationalEventType =
  | 'SHIFT_STARTED'
  | 'ROOM_OCCUPANCY_CHANGE'
  | 'ROOM_FLAGS_CHANGE'
  | 'ACUITY_CHANGE'
  | 'ASSIGNMENT_CHANGE'
  | 'STAFF_STATUS_CHANGE'
...
export interface ForecastEvent {
  id: string;
  procedureType: 'CABG' | 'Valve Surgery' | 'TAVR' | 'Cath/PCI' | 'EP' | 'Direct/ED Admission' | 'Expected Discharge' | 'Other';
  expectedTime: string;
  acuity: AcuityLevel;
