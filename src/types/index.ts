...
export interface CurrentShiftState { date: string; shiftType: 'Day' | 'Night'; roster: NurseStaff[]; rooms: PatientRoom[]; mtState: MTCoverageState; pctState: PCTCoverageState; onCall: OnCallProviders; lastUpdatedAt: string; }
export interface ForecastEvent { id: string; procedureType: 'CABG' | 'Valve Surgery' | 'TAVR' | 'Cath/PCI' | 'EP' | 'Direct/ED Admission' | 'Expected Discharge' | 'Other'; expectedTime: string; acuity: AcuityLevel; destinationLevel: string; notes: string; }
