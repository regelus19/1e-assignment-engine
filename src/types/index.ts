...
export type ComplexityFlag = 'Vent' | 'Pressors' | 'Vasoactive Support' | 'Inotropic Support' | 'Impella/IABP' | 'Fresh Post-Op' | 'Pending Surgery' | 'Pending Procedure' | 'HD/Dialysis' | 'Isolation' | 'Sitter/Safety' | 'High Fall Risk' | 'Confused' | 'Admission' | 'Transfer' | 'Possible DC' | 'Expected DC' | 'Expected Admission' | 'BLOCKED' | 'Discharge';
...
export interface AssignmentWarning { type: 'CAPABILITY' | 'DIALYSIS_ROOM' | 'WORKLOAD_RATIO' | 'GEOGRAPHY' | 'SAFETY_ROOM' | 'ACUITY_MISSING' | 'CONTINUITY'; severity: 'HIGH' | 'MEDIUM' | 'INFO'; message: string; roomNumber?: string; nurseName?: string; }
...
export interface PlanBaseline extends FinalizedShiftSnapshot { savedAt: string; pmOnCall?: OnCallProviders; }
...
export interface PlanningWorkspace { targetDate: string; shiftType: 'Day' | 'Night'; roster: NurseStaff[]; rooms: PatientRoom[]; mtState: MTCoverageState; pctState: PCTCoverageState; onCall: OnCallProviders; pmOnCall: OnCallProviders; currentMRS: number; projectedMRS: number; lastUpdatedAt: string; }
