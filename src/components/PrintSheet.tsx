...
const linkedPartner=(s:NurseStaff,roster:NurseStaff[])=>{const direct=s.orientationPartnerId?roster.find(x=>x.id===s.orientationPartnerId):undefined;return direct||roster.find(x=>x.orientationPartnerId===s.id&&['Preceptor','Orientee'].includes(x.role));};
const pairName=(s:NurseStaff,roster:NurseStaff[])=>{const p=linkedPartner(s,roster);return s.role==='Preceptor'&&p?`${first(s.name)} / ${first(p.name)}`:first(s.name);};
...
const ShiftTable:React.FC<{title:string;date:string;roster:NurseStaff[];rooms:PatientRoom[]}>=({title,date,roster,rooms})=>{
 ...
 return <section>
   <div className="flex justify-between items-end mb-1"><div className="text-[16px] font-black uppercase">{title}</div><div className="text-[16px] font-black">{date}</div></div>
   <table className="w-full border-collapse border-2 border-black text-[15px] leading-tight table-fixed">
     ...
     <tbody>{padded.map((r,i)=><tr key={r.id||i}><td className="border border-black px-2 py-[4px] font-bold h-[27px]">{r.name}</td><td className="border border-black px-2 py-[4px] font-semibold">{r.assignment}</td><td className="border border-black px-2 py-[4px] text-center font-semibold">{r.phone}</td></tr>)}</tbody>
   </table>
 </section>;
};
...
const OnCallTable:React.FC<{am:OnCallProviders;pm:OnCallProviders}>=({am,pm})=><section className="my-2">
 <table className="w-full border-collapse border-2 border-black text-[14px] leading-tight table-fixed">
  ...
  <tbody>
   <tr><td className="border border-black px-2 py-1 font-black">INTENSIVIST</td><td className="border border-black px-2 py-1 whitespace-pre-wrap min-h-[30px]">{provider(am.intensivist)}</td><td className="border border-black px-2 py-1 whitespace-pre-wrap">{provider(pm.intensivist)}</td></tr>
   <tr><td className="border border-black px-2 py-2 font-black">CARDIOTHORACIC</td><td className="border border-black px-2 py-2 whitespace-pre-wrap h-[46px] align-top">{provider(am.cardiothoracic)}</td><td className="border border-black px-2 py-2 whitespace-pre-wrap h-[46px] align-top">{provider(pm.cardiothoracic)}</td></tr>
   ...
   <tr><td className="border border-black px-2 py-2 font-black">HOSPITALIST</td><td className="border border-black px-2 py-2 whitespace-pre-wrap h-[42px] align-top">{provider(am.hospitalist)}</td><td className="border border-black px-2 py-2 whitespace-pre-wrap h-[42px] align-top">{provider(pm.hospitalist)}</td></tr>
  </tbody>
 </table>
</section>;
...
export const PrintSheet:React.FC<PrintSheetProps>=({date,shiftType,roster,rooms,onCall,previousShift,nextShift})=>{
 ...
 return <div className="bg-white text-black px-2 py-1 font-serif mx-auto print:p-0 print-sheet-portrait" style={{maxWidth:'7.8in'}}>
   <div className="text-center mb-1"><h1 className="text-[19px] font-black tracking-wide">1 EAST DAILY STAFFING ASSIGNMENTS</h1><div className="text-[13px] font-bold">Cardiac Universal Bed (CUB) Unit</div></div>
