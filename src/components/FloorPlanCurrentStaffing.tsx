...
const linkedPartner = (staff: NurseStaff, roster: NurseStaff[]) => {
  const direct = staff.orientationPartnerId ? roster.find(x => x.id === staff.orientationPartnerId) : undefined;
  if (direct) return direct;
  return roster.find(x => x.orientationPartnerId === staff.id && ['Preceptor','Orientee'].includes(x.role));
};
const pairLabel = (staff: NurseStaff, roster: NurseStaff[]) => {
  const partner = linkedPartner(staff, roster);
  if (!partner || staff.role !== 'Preceptor') return firstName(staff.name);
  return `${firstName(staff.name)} / ${firstName(partner.name)}`;
};
const isPairedOrientee = (staff: NurseStaff, roster: NurseStaff[]) => staff.role === 'Orientee' && linkedPartner(staff, roster)?.role === 'Preceptor';
...
export const FloorPlanCurrentStaffing: React.FC<Props> = ({ currentShift, onRoomChange, onAssignRoom, onRecallPrevious }) => {
  ...
  const rosterRows = useMemo(() => currentShift.roster.filter(s => !isPairedOrientee(s, currentShift.roster)), [currentShift.roster]);
  ...
  return <div className="grid grid-cols-1 2xl:grid-cols-[260px_minmax(0,1fr)_320px] gap-4 items-start">
      ...
      <div className="bg-white border-2 border-slate-300 rounded-xl overflow-hidden">
        <div className="px-3 py-2 bg-slate-900 text-white">
          <div className="flex items-center justify-between gap-2">
            <div><div className="text-sm font-black uppercase">Live Assignment Board</div><div className="text-[10px] text-slate-300">Paired Preceptor / Orientee staff display together as one assignment row. Only active bedside nurses can receive rooms.</div></div>
            <button type="button" onClick={clearAll} className="text-[10px] font-bold border border-slate-600 rounded px-2 py-1 hover:bg-slate-800">Clear All</button>
          </div>
          <button type="button" onClick={onRecallPrevious} className="mt-2 w-full flex items-center justify-center gap-1.5 text-[10px] font-black border border-indigo-300 bg-indigo-500/20 text-indigo-100 rounded px-2 py-1.5 hover:bg-indigo-500/30" title="Restore matching previous patient-to-nurse assignments without overwriting any rooms you already assigned manually"><RotateCcw className="w-3 h-3"/>Recall Previous Assignments</button>
        </div>
        <div className="divide-y divide-slate-100">{rosterRows.map(staff => {
          const rs = roomsFor(staff.id), selected = manualStaffId === staff.id, canAssign = canReceivePatients(staff), partner = linkedPartner(staff,currentShift.roster);
          ...
          return <div key={staff.id} className={`p-2.5 ${selected ? 'bg-indigo-50' : !canAssign ? 'bg-slate-50/70' : ''}`}>
            <div className="flex items-center justify-between gap-1"><button disabled={!canAssign} onClick={() => selectStaff(staff)} className="text-left disabled:cursor-default"><div className={`text-sm font-black ${selected ? 'text-indigo-800' : canAssign ? 'text-slate-900' : 'text-slate-500'}`}>{pairLabel(staff, currentShift.roster)} <span className="font-bold">• {staff.role==='Preceptor'&&partner?'Preceptor/Orientee':staff.role}</span></div><div className={`text-[9px] font-black ${selected ? 'text-indigo-700' : canAssign ? 'text-blue-700' : 'text-slate-400'}`}>{rowStatus}</div></button><div className="flex items-center gap-1">{canAssign && <span className="text-[10px] text-slate-500">{rs.length} pt</span>}{rs.length > 0 && <button type="button" onClick={() => clearStaff(staff.id)} className="text-[9px] font-bold text-rose-700 border border-rose-200 rounded px-1.5 py-0.5">Clear</button>}</div></div>
            <div className="flex flex-wrap gap-1 mt-1">{rs.map(r => <span key={r.roomNumber} className={`inline-flex items-center rounded border text-[11px] font-black ${ACUITY_STYLES[r.acuity].badge}`}><button onClick={() => setSelectedRoomNumber(r.roomNumber)} className="px-1.5 py-1 inline-flex items-center gap-0.5">{r.roomNumber}{dischargeHouse(r,'w-2.5 h-2.5')}</button><button title={`Unassign ${r.roomNumber}`} onClick={() => onAssignRoom(r.roomNumber, null)} className="px-1 border-l border-current/20">×</button></span>)}{!rs.length && <span className="text-[10px] text-slate-400">—</span>}</div>
          </div>;
        })}</div>
        <div className={`p-2.5 border-t ${unassigned.length ? 'bg-rose-50' : 'bg-emerald-50'}`}><div className={`text-[11px] font-black ${unassigned.length ? 'text-rose-800' : 'text-emerald-800'}`}>NEEDS ASSIGNMENT ({unassigned.length})</div><div className="flex flex-wrap gap-1 mt-1">{unassigned.map(r => <button key={r.roomNumber} onClick={() => setSelectedRoomNumber(r.roomNumber)} className="px-1.5 py-1 rounded border border-rose-300 bg-white text-[10px] font-black text-rose-800 inline-flex items-center gap-0.5">{r.roomNumber}{dischargeHouse(r,'w-2.5 h-2.5')}</button>)}{!unassigned.length && <span className="text-[10px] font-bold text-emerald-700">All assigned ✓</span>}</div></div>
      </div>
    </aside>

    <section className="min-w-0 space-y-3">
      {manualStaff && <div className="bg-indigo-50 border-2 border-indigo-400 rounded-xl px-4 py-3 flex justify-between gap-3"><div><div className="text-sm font-black uppercase text-indigo-900">Rapid Manual Assignment Active</div><div className="text-base text-indigo-900"><b>{pairLabel(manualStaff, currentShift.roster)}</b> selected — click occupied rooms in succession. Clicking one of the same nurse's rooms again unassigns it.</div></div><button onClick={() => setManualStaffId(null)} className="bg-white border border-indigo-300 rounded-lg px-3 py-2 text-sm font-bold text-indigo-800 flex items-center gap-1"><X className="w-3.5 h-3.5" />Done</button></div>}

      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b flex justify-between"><div><h2 className="font-black text-base">1 East Floor Plan — Current Staffing</h2><p className="text-xs text-slate-500">✓ assigned • red outline needs assignment • quick-flag icons appear beside room tiles • yellow house = Possible DC • green house = Expected DC.</p></div><div className="text-xs text-slate-500">Selected: <b>{selectedRoom?.roomNumber}</b></div></div>
        <div className="relative bg-slate-50 overflow-hidden" style={{ aspectRatio: '1365 / 1152' }}>
          ...
          {currentShift.rooms.map(r => {
            ...
            return <button key={r.roomNumber} onClick={() => roomClick(r)} className={`absolute -translate-x-1/2 -translate-y-1/2 w-[5.7%] min-w-[44px] rounded-md border-2 shadow-sm px-1 py-1 text-center ${style} ${selected ? 'ring-4 ring-slate-900/20 z-20' : 'z-10'} ${need ? 'outline outline-4 outline-rose-500/70' : ''} ${belongs ? 'ring-4 ring-indigo-500/70' : ''}`} style={{ left: `${pos.left}%`, top: `${pos.top}%` }}>
              <div className="font-black text-[12px] flex items-center justify-center gap-0.5">{r.roomNumber}{r.isOccupied && r.assignedNurseId ? ' ✓' : ''}</div>
              <div className="text-[9px] font-bold">{r.isOccupied ? (acuityMissing ? 'ACUITY?' : r.acuity) : 'EMPTY'}</div>
              {r.isOccupied && <div className={`text-[9px] truncate ${need ? 'font-black text-rose-800' : ''}`}>{nurse ? pairLabel(nurse, currentShift.roster) : 'UNASSIGNED'}</div>}
              {r.flags.includes('BLOCKED') && <div className="text-[8px] font-black text-rose-900">BLOCKED</div>}
      ...
      <div className={`rounded-xl border p-3 ${unassigned.length ? 'bg-rose-50 border-rose-300' : 'bg-emerald-50 border-emerald-300'}`}><div className="flex justify-between"><div><div className="text-sm font-black uppercase">Rooms Needing Assignment</div><div className="text-[11px] text-slate-600">Occupied rooms only.</div></div><div className="text-xl font-black">{unassigned.length}</div></div><div className="flex flex-wrap gap-1 mt-2">{unassigned.map(r => <button key={r.roomNumber} onClick={() => setSelectedRoomNumber(r.roomNumber)} className={`px-2 py-1 rounded border text-[11px] font-black inline-flex items-center gap-1 ${r.acuityConfirmed === false ? 'bg-amber-100 border-amber-300 text-amber-900' : ACUITY_STYLES[r.acuity].badge}`}>{r.roomNumber} • {r.acuityConfirmed === false ? 'ACUITY?' : r.acuity}{dischargeHouse(r)}</button>)}</div></div>

      <div className="bg-white border border-slate-200 rounded-xl p-3"><div className="flex items-center justify-between gap-2"><div><div className="text-sm font-black uppercase text-slate-900">On-Call / QGenda</div><div className="text-[10px] text-slate-500">Displayed from current on-call data. Automation target: QGenda → Outlook calendar → app data source.</div></div><span className="text-[10px] font-bold bg-amber-50 text-amber-800 border border-amber-200 rounded px-2 py-1">MANUAL SOURCE NOW</span></div><div className="grid grid-cols-2 md:grid-cols-5 gap-2 mt-2 text-[11px]">{Object.entries({ Intensivist: currentShift.onCall.intensivist, Cardiothoracic: currentShift.onCall.cardiothoracic, 'Acute MI': currentShift.onCall.acuteMI, Cardiology: currentShift.onCall.cardiology, Hospitalist: currentShift.onCall.hospitalist }).map(([label, value]) => <div key={label} className="bg-slate-50 border rounded-lg p-2"><div className="font-black text-[9px] uppercase text-slate-500">{label}</div><div className="font-bold text-slate-800 mt-0.5">{value || '—'}</div></div>)}</div></div>
    </section>

    <aside className="bg-white border border-slate-200 rounded-xl p-4 2xl:sticky 2xl:top-4">{selectedRoom && <>
      <div className="flex justify-between border-b pb-3"><div><div className="text-2xl font-black flex items-center gap-1">Room {selectedRoom.roomNumber}{selectedRoom.isOccupied && dischargeHouse(selectedRoom,'w-4 h-4')}</div><div className="text-xs text-slate-500">Live room detail</div></div><label className="text-xs font-bold"><input type="checkbox" checked={selectedRoom.isOccupied} onChange={e => onRoomChange({ ...selectedRoom, isOccupied: e.target.checked, assignedNurseId: e.target.checked ? selectedRoom.assignedNurseId : null })} /> Occupied</label></div>
      <div className="mt-3"><label className="text-xs uppercase font-black text-slate-500">Acuity</label>{selectedRoom.isOccupied && selectedRoom.acuityConfirmed === false && <div className="mb-1 text-[10px] font-black text-amber-700">ACUITY NOT SET — choose one below</div>}<select value={selectedRoom.acuity} onChange={e => setAcuity(e.target.value as AcuityLevel)} className={`w-full border rounded-lg px-3 py-2 text-base font-black ${selectedRoom.acuityConfirmed === false ? 'bg-amber-50 border-amber-400 text-amber-900' : ACUITY_STYLES[selectedRoom.acuity].badge}`}><option>CVICU</option><option>ICU</option><option>PCU</option><option>TELE</option></select></div>
      <div className="mt-3"><label className="text-xs uppercase font-black text-slate-500">Assigned RN</label><select value={selectedRoom.assignedNurseId || ''} onChange={e => onAssignRoom(selectedRoom.roomNumber, e.target.value || null)} className="w-full border rounded-lg px-3 py-2 text-base"><option value="">Unassigned</option>{assignableStaff.map(s => <option key={s.id} value={s.id}>{pairLabel(s, currentShift.roster)} — {s.capability} / {s.staffStatus}</option>)}</select>{assignedNurse && <div className="text-xs text-slate-500 mt-1">Current: <b>{pairLabel(assignedNurse, currentShift.roster)}</b></div>}</div>
      <div className="mt-3"><label className="text-xs uppercase font-black text-slate-500">Non-PHI Stay Token</label><input value={selectedRoom.patientStayId} onChange={e => onRoomChange({ ...selectedRoom, patientStayId: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-base" /></div>
      <div className="mt-4"><div className="text-sm font-black mb-2">Quick Flags</div><div className="grid grid-cols-2 gap-1.5">{FLAGS.map(({ flag, label, icon }) => <button key={flag} onClick={() => toggleFlag(flag)} className={`min-h-9 flex items-center gap-1.5 px-2 py-1.5 rounded-lg border text-[11px] font-bold text-left ${roomHasFlag(selectedRoom, flag) ? 'bg-blue-50 border-blue-400 text-blue-800' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>{icon}<span>{label}</span></button>)}</div></div>
    </>}</aside>
  </div>;
};
