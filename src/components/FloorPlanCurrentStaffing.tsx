...
const FLAGS: { flag: ComplexityFlag; label: string; icon: React.ReactNode }[] = [
  ...
  { flag: 'Transfer', label: 'Pending Transfer', icon: <MoveRight className="w-3.5 h-3.5" /> }, { flag: 'Possible DC', label: 'Possible DC', icon: <LogOut className="w-3.5 h-3.5" /> },
  { flag: 'Expected DC', label: 'Expected DC', icon: <LogOut className="w-3.5 h-3.5" /> }, { flag: 'BLOCKED', label: 'BLOCKED', icon: <Ban className="w-3.5 h-3.5" /> },
...
export const FloorPlanCurrentStaffing: React.FC<Props> = ({ currentShift, onRoomChange, onAssignRoom, onRecallPrevious }) => {
  ...
  const bedside = useMemo(() => currentShift.roster.filter(s => ['RN', 'CHG', 'Preceptor'].includes(s.role)), [currentShift.roster]);
  ...
  return <div className="grid grid-cols-1 2xl:grid-cols-[240px_minmax(0,1fr)_300px] gap-4 items-start">
    ...
    <section className="min-w-0 space-y-3">
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden"><div className="px-4 py-3 border-b flex justify-between"><div><h2 className="font-black text-sm">1 East Floor Plan — Current Staffing</h2><p className="text-[11px] text-slate-500">✓ assigned • red outline needs assignment • amber ACUITY? means the patient is still in census but needs a level-of-care code.</p></div><div className="text-[11px] text-slate-500">Selected: <b>{selectedRoom?.roomNumber}</b></div></div><div className="relative bg-slate-50 overflow-hidden" style={{ aspectRatio: '1365 / 1152' }}><img src={floorPlanImage} alt="1 East unit floor plan" className="absolute inset-0 w-full h-full object-contain opacity-75" />{currentShift.rooms.map(r => { const pos = ROOM_POSITIONS[r.roomNumber]; if (!pos) return null; const nurse = currentShift.roster.find(s => s.id === r.assignedNurseId), need = r.isOccupied && !r.assignedNurseId, selected = r.roomNumber === selectedRoom?.roomNumber, belongs = !!manualStaffId && r.assignedNurseId === manualStaffId, acuityMissing = r.isOccupied && r.acuityConfirmed === false, style = !r.isOccupied ? 'bg-white/90 border-slate-400 text-slate-500' : acuityMissing ? 'bg-amber-100 border-amber-500 text-amber-950' : ACUITY_STYLES[r.acuity].room; return <button key={r.roomNumber} onClick={() => roomClick(r)} className={`absolute -translate-x-1/2 -translate-y-1/2 w-[5.7%] min-w-[44px] rounded-md border-2 shadow-sm px-1 py-1 text-center ${style} ${selected ? 'ring-4 ring-slate-900/20 z-20' : 'z-10'} ${need ? 'outline outline-4 outline-rose-500/70' : ''} ${belongs ? 'ring-4 ring-indigo-500/70' : ''}`} style={{ left: `${pos.left}%`, top: `${pos.top}%` }}><div className="font-black text-[11px]">{r.roomNumber}{r.isOccupied && r.assignedNurseId ? ' ✓' : ''}</div><div className="text-[8px] font-bold">{r.isOccupied ? (acuityMissing ? 'ACUITY?' : r.acuity) : 'EMPTY'}</div>{r.isOccupied && <div className={`text-[8px] truncate ${need ? 'font-black text-rose-800' : ''}`}>{nurse ? pairLabel(nurse, currentShift.roster) : 'UNASSIGNED'}</div>}{r.flags.includes('BLOCKED') && <div className="text-[7px] font-black text-rose-900">BLOCKED</div>}</button>; })}</div></div>

      <div className={`rounded-xl border p-3 ${unassigned.length ? 'bg-rose-50 border-rose-300' : 'bg-emerald-50 border-emerald-300'}`}><div className="flex justify-between"><div><div className="text-xs font-black uppercase">Rooms Needing Assignment</div><div className="text-[10px] text-slate-600">Occupied rooms only.</div></div><div className="text-lg font-black">{unassigned.length}</div></div><div className="flex flex-wrap gap-1 mt-2">{unassigned.map(r => <button key={r.roomNumber} onClick={() => setSelectedRoomNumber(r.roomNumber)} className={`px-2 py-1 rounded border text-[10px] font-black ${r.acuityConfirmed === false ? 'bg-amber-100 border-amber-300 text-amber-900' : ACUITY_STYLES[r.acuity].badge}`}>{r.roomNumber} • {r.acuityConfirmed === false ? 'ACUITY?' : r.acuity}</button>)}</div></div>
    ...
    <aside className="bg-white border border-slate-200 rounded-xl p-4 2xl:sticky 2xl:top-4">{selectedRoom && <><div className="flex justify-between border-b pb-3"><div><div className="text-xl font-black">Room {selectedRoom.roomNumber}</div><div className="text-[11px] text-slate-500">Live room detail</div></div><label className="text-[10px] font-bold"><input type="checkbox" checked={selectedRoom.isOccupied} onChange={e => onRoomChange({ ...selectedRoom, isOccupied: e.target.checked, assignedNurseId: e.target.checked ? selectedRoom.assignedNurseId : null })} /> Occupied</label></div><div className="mt-3"><label className="text-[10px] uppercase font-black text-slate-500">Acuity</label>{selectedRoom.isOccupied && selectedRoom.acuityConfirmed === false && <div className="mb-1 text-[9px] font-black text-amber-700">ACUITY NOT SET — choose one below</div>}<select value={selectedRoom.acuity} onChange={e => setAcuity(e.target.value as AcuityLevel)} className={`w-full border rounded-lg px-3 py-2 text-sm font-black ${selectedRoom.acuityConfirmed === false ? 'bg-amber-50 border-amber-400 text-amber-900' : ACUITY_STYLES[selectedRoom.acuity].badge}`}><option>CVICU</option><option>ICU</option><option>PCU</option><option>TELE</option></select></div><div className="mt-3"><label className="text-[10px] uppercase font-black text-slate-500">Assigned RN</label><select value={selectedRoom.assignedNurseId || ''} onChange={e => onAssignRoom(selectedRoom.roomNumber, e.target.value || null)} className="w-full border rounded-lg px-3 py-2 text-sm"><option value="">Unassigned</option>{bedside.map(s => <option key={s.id} value={s.id}>{pairLabel(s, currentShift.roster)} — {s.capability} / {s.staffStatus}</option>)}</select>{assignedNurse && <div className="text-[10px] text-slate-500 mt-1">Current: <b>{pairLabel(assignedNurse, currentShift.roster)}</b></div>}</div><div className="mt-3"><label className="text-[10px] uppercase font-black text-slate-500">Non-PHI Stay Token</label><input value={selectedRoom.patientStayId} onChange={e => onRoomChange({ ...selectedRoom, patientStayId: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm" /></div><div className="mt-4"><div className="text-xs font-black mb-2">Quick Flags</div><div className="grid grid-cols-2 gap-1.5">{FLAGS.map(({ flag, label, icon }) => <button key={flag} onClick={() => toggleFlag(flag)} className={`min-h-9 flex items-center gap-1.5 px-2 py-1.5 rounded-lg border text-[10px] font-bold text-left ${selectedRoom.flags.includes(flag) ? 'bg-blue-50 border-blue-400 text-blue-800' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>{icon}<span>{label}</span></button>)}</div></div></>}</aside>
  </div>;
};