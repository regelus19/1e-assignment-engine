...
export const FloorPlanCurrentStaffing: React.FC<Props> = ({ currentShift, onRoomChange, onAssignRoom }) => {
  ...
  const setAcuity = (acuity: AcuityLevel) => selectedRoom && onRoomChange({ ...selectedRoom, acuity, isOccupied: true });
  ...
  return <div className="grid grid-cols-1 2xl:grid-cols-[240px_minmax(0,1fr)_300px] gap-4 items-start">
    ...
    <section className="min-w-0 space-y-3">
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden"><div className="px-4 py-3 border-b flex justify-between"><div><h2 className="font-black text-sm">1 East Floor Plan — Current Staffing</h2><p className="text-[11px] text-slate-500">✓ assigned • red outline needs assignment • Rapid Room Coding handles batch acuity/flags.</p></div><div className="text-[11px] text-slate-500">Selected: <b>{selectedRoom?.roomNumber}</b></div></div><div className="relative bg-slate-50 overflow-hidden" style={{ aspectRatio: '1365 / 1152' }}><img src={floorPlanImage} alt="1 East unit floor plan" className="absolute inset-0 w-full h-full object-contain opacity-75" />{currentShift.rooms.map(r => { const pos = ROOM_POSITIONS[r.roomNumber]; if (!pos) return null; const nurse = currentShift.roster.find(s => s.id === r.assignedNurseId), need = r.isOccupied && !r.assignedNurseId, selected = r.roomNumber === selectedRoom?.roomNumber, belongs = !!manualStaffId && r.assignedNurseId === manualStaffId, style = r.isOccupied ? ACUITY_STYLES[r.acuity].room : 'bg-white/90 border-slate-400 text-slate-500'; return <button key={r.roomNumber} onClick={() => roomClick(r)} className={`absolute -translate-x-1/2 -translate-y-1/2 w-[5.7%] min-w-[44px] rounded-md border-2 shadow-sm px-1 py-1 text-center ${style} ${selected ? 'ring-4 ring-slate-900/20 z-20' : 'z-10'} ${need ? 'outline outline-4 outline-rose-500/70' : ''} ${belongs ? 'ring-4 ring-indigo-500/70' : ''}`} style={{ left: `${pos.left}%`, top: `${pos.top}%` }}><div className="font-black text-[11px]">{r.roomNumber}{r.isOccupied && r.assignedNurseId ? ' ✓' : ''}</div><div className="text-[8px] font-bold">{r.isOccupied ? r.acuity : 'EMPTY'}</div>{r.isOccupied && <div className={`text-[8px] truncate ${need ? 'font-black text-rose-800' : ''}`}>{nurse ? pairLabel(nurse, currentShift.roster) : 'UNASSIGNED'}</div>}{r.flags.includes('BLOCKED') && <div className="text-[7px] font-black text-rose-900">BLOCKED</div>}</button>; })}</div></div>

      <div className={`rounded-xl border p-3 ${unassigned.length ? 'bg-rose-50 border-rose-300' : 'bg-emerald-50 border-emerald-300'}`}><div className="flex justify-between"><div><div className="text-xs font-black uppercase">Rooms Needing Assignment</div><div className="text-[10px] text-slate-600">Occupied rooms only.</div></div><div className="text-lg font-black">{unassigned.length}</div></div><div className="flex flex-wrap gap-1 mt-2">{unassigned.map(r => <button key={r.roomNumber} onClick={() => setSelectedRoomNumber(r.roomNumber)} className={`px-2 py-1 rounded border text-[10px] font-black ${ACUITY_STYLES[r.acuity].badge}`}>{r.roomNumber} • {r.acuity}</button>)}</div></div>
