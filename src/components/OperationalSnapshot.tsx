...
export const OperationalSnapshot: React.FC<Props> = ({ currentCensus, rooms, capacity = 22 }) => {
  ...
  return <section className="bg-white border border-slate-200 rounded-xl no-print shadow-sm overflow-hidden">
    <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2"><div><div className="flex items-center gap-2"><Building2 className="w-4 h-4 text-blue-700"/><h2 className="text-xs font-black uppercase tracking-wider text-slate-800">1E Operational Snapshot</h2></div><p className="text-[10px] text-slate-500 mt-0.5">Today's patient flow • discharge counts are driven by room flags; incoming flow remains manual in V1.</p></div><button type="button" onClick={() => { localStorage.setItem(KEY, JSON.stringify(data)); setSaved(true); }} className="px-2.5 py-1.5 rounded-lg border border-slate-300 bg-white text-[10px] font-bold flex items-center gap-1"><Save className="w-3 h-3"/>{saved ? 'Saved' : 'Save Flow Inputs'}</button></div>
    <div className="p-3"><div className="flex flex-wrap gap-2 items-stretch">
      <ReadOnlyMetric label="Current Census" value={currentCensus} note={`of ${capacity} beds`} emphasis="bg-slate-900 border-slate-900 text-white" />
      <ReadOnlyMetric label="Beds Available" value={availableNow} note="right now" />
    ...
    <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px]"><span className="inline-flex items-center gap-1 bg-blue-50 text-blue-800 border border-blue-200 rounded-full px-2 py-1"><ArrowDownToLine className="w-3 h-3"/>Incoming <b>{incoming}</b></span><span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-800 border border-emerald-200 rounded-full px-2 py-1"><ArrowUpFromLine className="w-3 h-3"/>Expected leaving <b>{leaving}</b></span><span className="inline-flex items-center gap-1 bg-slate-50 text-slate-800 border border-slate-200 rounded-full px-2 py-1"><ArrowRightLeft className="w-3 h-3"/>Net flow <b>{incoming - leaving >= 0 ? '+' : ''}{incoming - leaving}</b></span><span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 border font-black ${pressure === 'ALERT' ? 'bg-rose-50 text-rose-800 border-rose-300' : pressure === 'WATCH' ? 'bg-amber-50 text-amber-800 border-amber-300' : 'bg-emerald-50 text-emerald-800 border-emerald-300'}`}><Building2 className="w-3 h-3"/>Projected Census {projectedCensus}/{capacity} • {pressure}</span><span className="inline-flex items-center gap-1 bg-slate-50 text-slate-800 border border-slate-200 rounded-full px-2 py-1"><BedDouble className="w-3 h-3"/>Projected available <b>{projectedAvailable}</b></span>{deficit > 0 && <span className="font-black text-rose-700">Projected deficit: {deficit} bed{deficit === 1 ? '' : 's'}</span>}</div></div>
  </section>;
};
