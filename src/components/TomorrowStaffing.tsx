...
export const TomorrowStaffing:React.FC<Props>=({date,shiftType,roster,rooms,mtState,pctState,onCall,onDateChange,onShiftTypeChange,onRosterChange,onRoomsChange,onMtStateChange,onPctStateChange,onSaveBaseline})=>{
  ...
  const fixedAssignments=Object.fromEntries(rooms.filter(r=>r.isOccupied&&r.assignedNurseId&&!recalledRooms.has(r.roomNumber)).map(r=>[r.roomNumber,r.assignedNurseId as string]));
  ...
  const resetRecallContext=()=>{setRecalledRooms(new Set());setRecallMessage('');setRecommendationOptions([]);};
  ...
  const recallPreviousAssignments=()=>{
    ...
    setRecalledRooms(prev=>new Set([...prev,...recalledNow]));
    onRoomsChange(next);
    const message=recalled
      ? `Recalled ${recalled} assignment${recalled===1?'':'s'} from shifts before ${date} ${shiftType}. Continuity follows Patient Stay Token to the returning nurse — not the room number. Manual assignments were preserved.`
  ...
  const generate=(semiAuto=false)=>{
    // Recalled rooms are continuity anchors, not hard-closed CN loads. We deliberately exclude them
    // from fixedAssignments so the engine can safely complete that nurse's load. The engine's strong
    // continuity score should retain the returning patient/nurse relationship unless safety prevents it.
  ...
  const applyOption=(option:RecommendationOption)=>{
    ...
    setWarnings(result.warnings);setUnassigned(result.unassignedRooms);setFitLabel(result.fitLabel);setRecommendationOptions([]);setSemiAutoActive(false);setRecalledRooms(new Set());
  };

  return <div className="space-y-5">
    <div className="bg-slate-900 text-white rounded-xl p-4"><div className="flex flex-wrap justify-between gap-4"><div><div className="flex items-center gap-2"><CalendarDays className="w-4 h-4 text-blue-300"/><span className="text-xs uppercase font-black text-slate-300">Next Shift Plan</span></div><div className="text-xl font-black mt-1">{date} • {shiftType} Shift</div><div className="text-xs text-slate-400 mt-1">Auto first. If the CN wants more control, Recall Previous / manual anchors → Semi-Auto Fill Rest.</div></div><div className="flex flex-wrap gap-2 text-xs"><div className="bg-slate-800 border border-slate-700 rounded px-3 py-2">Planned Census <b>{census}</b></div><div className="bg-slate-800 border border-slate-700 rounded px-3 py-2">Active bedside RNs <b>{activeRNs}</b></div><div className="bg-slate-800 border border-slate-700 rounded px-3 py-2">Reserve <b>{reserve}</b></div><div className="bg-slate-800 border border-slate-700 rounded px-3 py-2">Recommendation <b>{fitLabel}</b></div></div></div></div>
    ...
    {semiAutoReadyCount>0&&<div className="bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-3 text-xs text-indigo-900"><b>Semi-Auto ready:</b> recalled patients are continuity anchors; nurses with recalled patients may still receive an appropriate additional patient. Rooms you manually assign after recall are CN-fixed loads.</div>}
