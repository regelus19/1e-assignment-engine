...
import { AlertTriangle, Clock3, Play, RefreshCw, Save, Users } from 'lucide-react';
import {
  AssignmentWarning,
  CurrentShiftState,
  NurseStaff,
...
interface Props {
  ...
  onCurrentShiftChange: (state: CurrentShiftState) => void;
  events: OperationalEvent[];
  onEventsChange: (events: OperationalEvent[]) => void;
...
export const CurrentStaffing: React.FC<Props> = ({ currentShift, onCurrentShiftChange, events, onEventsChange, onStartFromPlan }) => {
  ...
  const activeRNs = currentShift.roster.filter(s => s.role === 'RN' && ['ACTIVE', 'RECALLED'].includes(s.staffStatus)).length;
  const onCallCount = currentShift.roster.filter(s => ['ON_CALL', 'FLEXED'].includes(s.staffStatus)).length;

  const appendEvent = (event: OperationalEvent) => {
    StorageService.appendOperationalEvent(event);
    onEventsChange(StorageService.loadOperationalEvents());
  };
  ...
  const updateRoom = (updated: PatientRoom) => {
    const before = currentShift.rooms.find(r => r.roomNumber === updated.roomNumber);
  ...
  const assignRoom = (roomNumber: string, nurseId: string | null) => {
    const before = currentShift.rooms.find(r => r.roomNumber === roomNumber)?.assignedNurseId || null;
    const nextRooms = currentShift.rooms.map(r => r.roomNumber === roomNumber ? { ...r, assignedNurseId: nurseId } : r);
  ...
  const generateCurrentRecommendation = () => {
    const result = runRecommendationEngine(currentShift.roster, currentShift.rooms, false, false);
    const nextRooms = currentShift.rooms.map(r => ({ ...r, assignedNurseId: result.assignments[r.roomNumber] || null }));
    saveState({ ...currentShift, rooms: nextRooms });
