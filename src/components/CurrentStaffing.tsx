...
import { AlertTriangle, Clock3, Download, Play, RefreshCw, Save, Settings2, Sparkles } from 'lucide-react';
...
export const CurrentStaffing:React.FC<Props>=({currentShift,onCurrentShiftChange,events,onEventsChange,onStartFromPlan})=>{
 ...
 const recallPreviousAssignments=()=>{
   let recalled=0;
   const nextRooms=currentShift.rooms.map(room=>{
     if(!room.isOccupied||room.assignedNurseId||!room.patientStayId) return room;
     const continuity=StorageService.findContinuity(room.patientStayId,currentShift.roster);
     if(!continuity) return room;
     recalled+=1;
     return {...room,assignedNurseId:continuity.nurseId};
   });
   saveState({...currentShift,rooms:nextRooms});
   appendEvent({id:`evt-${Date.now()}-recall`,timestamp:new Date().toISOString(),shiftDate:currentShift.date,shiftType:currentShift.shiftType,type:'ASSIGNMENT_CHANGE',summary:`Recalled ${recalled} previous patient assignment${recalled===1?'':'s'} into the Live Assignment Board. Existing CN manual assignments were preserved.`});
 ...
 const finalizeShift=()=>{
   ...
   appendEvent({id:`evt-${Date.now()}-finalize`,timestamp:finalizedAt,shiftDate:currentShift.date,shiftType:currentShift.shiftType,type:'ASSIGNMENT_CHANGE',summary:`Finalized and saved ${currentShift.date} ${currentShift.shiftType} shift to history for handoff and continuity recall.`});
   setFitLabel('SHIFT SAVED');
   window.alert(`${currentShift.date} ${currentShift.shiftType} shift saved to History.`);
 };
 const exportShiftCsv=()=>{
   const headers=['Shift Date','Shift','Exported At','Room','PatientStayID','Acuity','Flags','Assigned Nurse','Role','Capability','Staff Status','Phone','MT Coverage','PCT Support','Clinical Supervisor'];
   const rows=currentShift.rooms.filter(r=>r.isOccupied).map(room=>{
     const nurse=currentShift.roster.find(s=>s.id===room.assignedNurseId);
     return [currentShift.date,currentShift.shiftType,new Date().toISOString(),room.roomNumber,room.patientStayId,room.acuity,room.flags.join(' | '),nurse?.name||'',nurse?.role||'',nurse?.capability||'',nurse?.staffStatus||'',nurse?.assignedPhone||'',currentShift.mtState,currentShift.pctState,currentShift.clinicalSupervisorPresent?'Present':'Not Present'];
   });
   const csv='\ufeff'+[headers,...rows].map(row=>row.map(csvCell).join(',')).join('\r\n');
   const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});
   const url=URL.createObjectURL(blob);
   const a=document.createElement('a');
   a.href=url;a.download=`1E_Assignment_${currentShift.date}_${currentShift.shiftType}.csv`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);
 };
 ...
 return <div className="space-y-5"><OperationalSnapshot currentCensus={census} rooms={currentShift.rooms} capacity={22}/>
  ...
  <FloorPlanCurrentStaffing currentShift={currentShift} onRoomChange={updateRoom} onAssignRoom={assignRoom} onStaffStatusChange={updateStatus} onRecallPrevious={recallPreviousAssignments}/>
