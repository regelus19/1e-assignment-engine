...
export const RapidRoomTools:React.FC<Props>=({rooms,onRoomChange})=>{
  ...
  return <div className="bg-white border border-slate-200 rounded-xl p-3 space-y-3">
    ...
    {tool && <div className="border-t pt-2"><div className="text-[9px] font-black text-slate-700 mb-1">{tool.kind==='acuity'?`Set / toggle ${tool.value} for:`:tool.kind==='status'?`Mark rooms ${tool.value==='OCCUPIED'?'occupied':'vacant'}:`:`Toggle ${flags.find(f=>f.value===tool.value)?.label} for:`}</div><div className="flex flex-wrap gap-1">{targetRooms.map(r=>{const active=tool.kind==='acuity'?(r.acuityConfirmed!==false&&r.acuity===tool.value):tool.kind==='status'?(tool.value==='OCCUPIED'?r.isOccupied:!r.isOccupied):r.flags.includes(tool.value);return <button key={r.roomNumber} onClick={()=>apply(r)} className={`rounded border px-1.5 py-1 text-[9px] font-black ${active?'bg-slate-800 text-white border-slate-800':'bg-white text-slate-700 border-slate-300'}`}>{r.roomNumber}{!r.isOccupied?' · V':''}{active?' ✓':''}</button>})}</div><div className="text-[8px] text-slate-500 mt-1">V = vacant. Coding acuity on a vacant room preserves vacancy; it does not count toward census. Marking a room Vacant also clears its RN assignment.</div><button onClick={()=>setTool(null)} className="mt-2 text-[9px] font-bold text-slate-500 underline">Clear rapid tool</button></div>}
