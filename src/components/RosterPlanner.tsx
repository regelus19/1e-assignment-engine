    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center">
          <div>
            <h3 className="font-black text-sm uppercase tracking-wide">Tomorrow's Staffing Roster</h3>
            <p className="text-xs text-slate-500 mt-1">Use first names for planning. Capability and operational status drive recommendations.</p>
          </div>
            ...
            <tbody>
              {roster.map(staff => (
                <tr key={staff.id} className="border-t border-slate-100">
                  ...
                  <td className="p-2"><select value={staff.role} onChange={e => update(staff.id, { role: e.target.value as StaffRole })} className="border rounded px-2 py-1.5"><option>CHG</option><option>RN</option><option>MT</option><option>PCT</option><option>Resource</option></select></td>
