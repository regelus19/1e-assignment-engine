...
export const StorageService = {
  loadStaff(): NurseStaff[] {
    const raw = localStorage.getItem(ROSTER_KEY);
    return raw ? JSON.parse(raw) : clone(INITIAL_STAFF);
  },
  saveStaff(staff: NurseStaff[]): void {
    localStorage.setItem(ROSTER_KEY, JSON.stringify(staff));
  },
  loadRooms(): PatientRoom[] {
    const raw = localStorage.getItem(ROOMS_KEY);
    return raw ? JSON.parse(raw) : clone(INITIAL_ROOMS);
  },
  saveRooms(rooms: PatientRoom[]): void {
    localStorage.setItem(ROOMS_KEY, JSON.stringify(rooms));
  },
  loadOnCall(): OnCallProviders {
    const raw = localStorage.getItem(ONCALL_KEY);
    return raw ? JSON.parse(raw) : clone(INITIAL_ONCALL);
  },
  saveOnCall(onCall: OnCallProviders): void {
    localStorage.setItem(ONCALL_KEY, JSON.stringify(onCall));
  },
  loadHistory(): FinalizedShiftSnapshot[] {
    const raw = localStorage.getItem(HISTORY_KEY);
    return raw ? JSON.parse(raw) : clone(INITIAL_HISTORY);
  },
  ...
  loadForecast(): ForecastEvent[] {
    const raw = localStorage.getItem(FORECAST_KEY);
    return raw ? JSON.parse(raw) : clone(INITIAL_FORECAST);
  },
  saveForecast(events: ForecastEvent[]): void {
    localStorage.setItem(FORECAST_KEY, JSON.stringify(events));
  },
  savePlanBaseline(snapshot: PlanBaseline): void {
    localStorage.setItem(PLAN_BASELINE_KEY, JSON.stringify(snapshot));
  },
  loadPlanBaseline(): PlanBaseline | null {
    const raw = localStorage.getItem(PLAN_BASELINE_KEY);
    return raw ? JSON.parse(raw) : null;
  },
  loadCurrentShift(): CurrentShiftState {
    const raw = localStorage.getItem(CURRENT_SHIFT_KEY);
    return raw ? JSON.parse(raw) : defaultCurrentShift();
  },
  saveCurrentShift(state: CurrentShiftState): void {
    localStorage.setItem(CURRENT_SHIFT_KEY, JSON.stringify({ ...state, lastUpdatedAt: new Date().toISOString() }));
  },
  startCurrentShiftFromPlan(plan: PlanBaseline): CurrentShiftState {
    const state: CurrentShiftState = {
      date: plan.date,
      shiftType: plan.shiftType,
      roster: clone(plan.roster),
      rooms: clone(plan.rooms),
      mtState: plan.mtState,
      pctState: plan.pctState,
      onCall: clone(plan.onCall),
      lastUpdatedAt: new Date().toISOString(),
    };
    this.saveCurrentShift(state);
    this.appendOperationalEvent({
      id: `evt-${Date.now()}`,
      timestamp: new Date().toISOString(),
      shiftDate: state.date,
      shiftType: state.shiftType,
      type: 'SHIFT_STARTED',
      summary: 'Current staffing initialized from saved plan baseline.',
    });
    return state;
  },
  loadOperationalEvents(): OperationalEvent[] {
    const raw = localStorage.getItem(OPERATIONAL_EVENTS_KEY);
    return raw ? JSON.parse(raw) : [];
  },
  appendOperationalEvent(event: OperationalEvent): void {
    const events = this.loadOperationalEvents();
    events.unshift(event);
    localStorage.setItem(OPERATIONAL_EVENTS_KEY, JSON.stringify(events.slice(0, 300)));
  },
  resetToDefaults(): void {
    [ROSTER_KEY, ROOMS_KEY, ONCALL_KEY, HISTORY_KEY, FORECAST_KEY, CURRENT_SHIFT_KEY, OPERATIONAL_EVENTS_KEY, PLAN_BASELINE_KEY].forEach(key => localStorage.removeItem(key));
  },
  findContinuity(patientStayId: string, currentRoster: NurseStaff[]): { nurseId: string; nurseName: string; daysAgo: number } | null {
